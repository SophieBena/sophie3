...
#ifdef DEBUG
		NSLog(@"Error in redlichKwong(). Case II encountered!");
#endif
		Z[0] = (double) 1.0;
		Z[1] = (double) 1.0;
		return;
...
#ifdef DEBUG
	NSLog(@"Error in redlichKwong(). Bad exit value!");
#endif
				...
				double  **dadx)    /* Mask: 100000000000000000000000 [NA][NA]     */
{
	...
	rhn = p/(zRK[0]*R*t);
#ifdef DEBUG
	NSLog(@"Redlich-Kwong rho: %14.6g", p/(zRK[0]*R*t));
	if (zRK[0] != zRK[1]) NSLog(@" rho: %14.6g", p/(zRK[1]*R*t));
#endif
...
#ifdef DEBUG
	NSLog(@"count, rh, dp, Z: %d %14.6g %14.6g %14.6g", count, rh, dp, p/(rh*R*t));
#endif
...
@implementation LiquidpMeltsH2O
...
-(id)init {
	if ((self = [super init])) {
		correction = [[LiquidpMeltsGenericEM alloc] initWithH:corrH19
															S:corrS19
														   k0:0.0
														   k1:0.0
														   k2:0.0
...
-(double)calculateWithT:(double)t andWithP:(double)p forCase:(NSUInteger)returnMode {
	...
	fluidPhase(t, 9550.0, x, &gH2O, NULL, NULL, NULL, &hH2O, &sH2O, NULL, NULL, &cpH2O, &dcpdtH2O, NULL,
			   &vH2O, NULL, NULL, &dvdtH2O, &dvdpH2O, &d2vdt2H2O, &d2vdtdpH2O, &d2vdp2H2O, NULL, NULL, NULL, NULL, NULL);
...
-(double)getGibbsFreeEnergyFromT:(double)t andP:(double)p {
	return [correction getGibbsFreeEnergyFromT:t andP:p] + [self calculateWithT:t andWithP:p forCase:returnValueOfG];
}

-(double)getEnthalpyFromT:(double)t andP:(double)p {
	return [correction getEnthalpyFromT:t andP:p] + [self calculateWithT:t andWithP:p forCase:returnValueOfH];
}

-(double)getEntropyFromT:(double)t andP:(double)p {
	return [correction getEntropyFromT:t andP:p] + [self calculateWithT:t andWithP:p forCase:returnValueOfS];
}

-(double)getHeatCapacityFromT:(double)t andP:(double)p {
	return [correction getHeatCapacityFromT:t andP:p] + [self calculateWithT:t andWithP:p forCase:returnValueOfCP];
}

-(double)getDcpDtFromT:(double)t andP:(double)p {
	return [correction getDcpDtFromT:t andP:p] + [self calculateWithT:t andWithP:p forCase:returnValueOfDCPDT];
}

-(double)getVolumeFromT:(double)t andP:(double)p {
	return  [correction getVolumeFromT:t andP:p];
}

-(double)getDvDtFromT:(double)t andP:(double)p {
	return [correction getDvDtFromT:t andP:p];
}

-(double)getDvDpFromT:(double)t andP:(double)p {
	return  [correction getDvDpFromT:t andP:p];
}

-(double)getD2vDt2FromT:(double)t andP:(double)p {
	return [correction getD2vDt2FromT:t andP:p];
}

-(double)getD2vDtDpFromT:(double)t andP:(double)p {
	return [correction getD2vDtDpFromT:t andP:p];
}

-(double)getD2vDp2FromT:(double)t andP:(double)p {
	return [correction getD2vDp2FromT:t andP:p];
}

@end

@implementation WaterpMelts
...
-(double)calculateWithT:(double)t andWithP:(double)p forCase:(NSUInteger)returnMode {
	...
	fluidPhase(t, p, x, &gH2O, NULL, NULL, NULL, &hH2O, &sH2O, NULL, NULL, &cpH2O, &dcpdtH2O, NULL,
			   &vH2O, NULL, NULL, &dvdtH2O, &dvdpH2O, &d2vdt2H2O, &d2vdtdpH2O, &d2vdp2H2O, NULL, NULL, NULL, NULL, NULL);

	switch (returnMode) {
		case 1:
			return hH2O - t*sH2O; /* used gH2O for calibration */
			break;
		case 2:
			return hH2O;
			break;
