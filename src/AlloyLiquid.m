...
+(void)initialize {
	if (self == [AlloyLiquid class]) {
		BOOL debug = false; // [[NSUserDefaults standardUserDefaults] boolForKey:@"DEBUG.SATLOOP.VERBOSE"];
		if (debug) NSLog(@"Initialize(AlloyLiquid) - entry ...");
		NSMutableArray *mutableEndmembers = [NSMutableArray arrayWithCapacity:NA];

		FeLiquid *feLiquid = [[FeLiquid alloc] init];
		[feLiquid setPhaseFormula:@"Fe"];
		[feLiquid setPhaseName:@"Fe liquid"];
		[mutableEndmembers addObject:feLiquid];
		if (debug) NSLog(@"... allocated Fe liquid ...");

		NiLiquid *niLiquid = [[NiLiquid alloc] init];
		[niLiquid setPhaseFormula:@"Ni"];
		[niLiquid setPhaseName:@"Ni liquid"];
		[mutableEndmembers addObject:niLiquid];
		if (debug) NSLog(@"... allocated Ni liquid ...");
...
-(id)init {
	if ((self = [super init])) {
		BOOL debug = false; // [[NSUserDefaults standardUserDefaults] boolForKey:@"DEBUG.SATLOOP.VERBOSE"];
		if (debug) NSLog(@"init(AlloyLiquid) ...");

		[self setPhaseName:@"Liquid Alloy"];
 ...
 *    The function conAlL defines the conversion from m[i], to r[j]
 */
...
{
	const char *NAMES[NA]    = { "Fe-liquid", "Ni-liquid" };
