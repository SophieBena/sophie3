...
+(void)initialize {
	if (self == [LeuciteBerman class]) {
		...
		BermanProperties *leucite = [[BermanProperties alloc] initWithH:-3032146.03+21866.4
																	  S:184.096+21.75
																	 k0:271.14
																	 k1:-9.441E2
																	 k2:-78.572E5
																 ...
																 deltaH:256.0
																	 ...
																	 v3:12.5167877e-6
																	 v4:0.0];
		[leucite setPhaseFormula:@"KAlSi2O6"];
		[leucite setPhaseName:@"leucite"];
		[mutableEndmembers addObject:leucite];
		if (debug) NSLog(@"... allocated leucite ...");

		BermanProperties *analcime = [[BermanProperties alloc] initWithH:-3308500.0-17400.0
																	   S:228.10
																	  k0:571.83
																	  k1:-71.887E2
																	  k2:0.0
																	  ...
																	  v3:12.5167877e-6
																	  v4:0.0];
		[analcime setPhaseFormula:@"NaAlSi2O5(OH)2"];
		[analcime setPhaseName:@"analcime"];
		[mutableEndmembers addObject:analcime];
		if (debug) NSLog(@"... allocated analcime ...");

		BermanProperties *sodaleucite = [[BermanProperties alloc] initWithH:-2980100.0-21875.55
																		  S:173.68-7.94
																		 k0:401.27
																		 k1:-42.480E2
																		 k2:0.0
																		 ...
																		 v3:12.5167877e-6
																		 v4:0.0];
		[sodaleucite setPhaseFormula:@"NaAlSi2O6"];
		[sodaleucite setPhaseName:@"sodaleucite"];
		[mutableEndmembers addObject:sodaleucite];
		if (debug) NSLog(@"... allocated sodaleucite ...");
...
-(id)init {
	if ((self = [super init])) {
		BOOL debug = false; // [[NSUserDefaults standardUserDefaults] boolForKey:@"DEBUG.SATLOOP.VERBOSE"];
		if (debug) NSLog(@"init(LeuciteBerman) ... entry ...");
		...
		if (isMELTS) {
			if (debug) NSLog(@"LeuciteBerman is configured for MELTS.");
		} else {
			if (debug) NSLog(@"LeuciteBerman is configured for pMELTS.");
			BermanProperties *leucite = [endmembers objectAtIndex:0];
			[leucite setH:-3032146.03+27000.0];
		}
 ...
 */
#define FR0(i)     (i == 0) ? 1.0 - r[0] : - r[0]
#define FR1(i)     (i == 1) ? 1.0 - r[1] : - r[1]
		   ...
		   d3m:(double [NR][NA][NA][NA])d3m // 3rd deriv matrix: d3m[i][j][k][l]=d3r[i]/dm[j]dm[k]dm[l]
{
	...
	if (inpMask == FIRST && outMask == SECOND) {
		...
		if (outMask & ~(THIRD | FOURTH | FIFTH | SIXTH | EIGHTH))
			NSLog(@"Illegal call to conLeu with inpMask = %o and outMask = %o", inpMask, outMask);
						  ...
						  r:(double [NA])r
{
	...
	return [NSString stringWithFormat:@"K%4.2fNa%4.2fAlSi2O%4.2f(OH)%4.2f", totK, totNa, totO, totOH];
}
...
{
	double xKL   = (r[0]*(1.0-r[1])         > DBL_EPSILON) ? r[0]*(1.0-r[1])         : DBL_EPSILON;
	double xNaL  = ((1.0-r[0])*(1.0-r[1])   > DBL_EPSILON) ? (1.0-r[0])*(1.0-r[1])   : DBL_EPSILON;
	double xH2OL = (r[1]                    > DBL_EPSILON) ? r[1]                    : DBL_EPSILON;
	double xKS   = (2.0*r[0]*r[1]/3.0       > DBL_EPSILON) ? 2.0*r[0]*r[1]/3.0       : DBL_EPSILON;
	double xNaS  = (2.0*(1.0-r[0])*r[1]/3.0 > DBL_EPSILON) ? 2.0*(1.0-r[0])*r[1]/3.0 : DBL_EPSILON;
	double xVcS  = ((1.0-2.0*r[1]/3.0)      > DBL_EPSILON) ? (1.0-2.0*r[1]/3.0)      : DBL_EPSILON;
...
-(DoubleVector *)chemicalPotentialsOfSpeciesFromMolesOfComponents:(double *)m andT:(double)t andP:(double)p {
	...
	// KAlSi2O5(OH)2
	muSpecies[3] = muSpecies[1] + muSpecies[0] - muSpecies[2];
...
-(NSArray *)affinityAndCompositionFromLiquidChemicalPotentialSum:(double *)chemicalPotentials andT:(double)t andP:(double)p {
	...
	if ((chemicalPotentials[0] != 0.0) && (chemicalPotentials[1] != 0.0) && (chemicalPotentials[2] != 0.0)) {
		mu0[3] = mu0[1] + mu0[0] - mu0[2] + (DGR);
		deltaMu[nz] = chemicalPotentials[1] + chemicalPotentials[0] - chemicalPotentials[2] - mu0[3];
		index[nz] = 3;
		gamma[nz] = 1.0;
		nz++;
	} else mu0[3] = 0.0;
	...
	do {
		...
		if ((chemicalPotentials[0] != 0.0) && (chemicalPotentials[1] != 0.0) && (chemicalPotentials[2] != 0.0)) {
			a[3] = exp((mu[0] + mu[1] - mu[2] + mu0[0] + mu0[1] - mu0[2] - mu0[3])/(R*t));
			gamma[j] = a[3]/x[3];
		} else gamma[j] = 0.0;
	...
	if (debugS) {
		...
		NSLog(@"... ... Activity coefficient of %@ is %13.6g with mole fraction %13.6g", @"potashanalcime ", gamma[j], x[3]);
	}
