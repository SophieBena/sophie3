...
+(void)initialize {
	if (self == [BiotiteBerman class]) {
		...
		BermanProperties *annite = [[BermanProperties alloc] initWithH:-5142800.0
																	...
																	v3:34.4473262e-6
																	v4:0.0];
		[annite setPhaseFormula:@"KFe3Si3AlO10(OH)2"];
		[annite setPhaseName:@"annite"];
		[mutableEndmembers addObject:annite];
		if (debug) NSLog(@"... allocated annite ...");

		BermanProperties *phlogopite = [[BermanProperties alloc] initWithH:-6210391.0
																		 S:334.346
																		k0:610.37988
																		k1:-20.83781e2
																		k2:-215.33008e5
																		...
																		v3:34.4473262e-6
																		v4:0.0];
		[phlogopite setPhaseFormula:@"KMg3Si3AlO10(OH)2"];
		[phlogopite setPhaseName:@"phlogopite"];
		[mutableEndmembers addObject:phlogopite];
		if (debug) NSLog(@"... allocated phlogopite ...");
...
-(id)init {
	if ((self = [super init])) {
		BOOL debug = false; // [[NSUserDefaults standardUserDefaults] boolForKey:@"DEBUG.SATLOOP.VERBOSE"];
		if (debug) NSLog(@"init(BiotiteBerman) ...");

		[self setPhaseName:@"Biotite"];
 ...
 * Sack, R.O., Ghiorso, M.S.
 * Importance of considerations of mixing properties in establishing an
 * internally consistent thermodynamic database: thermochemistry of
 * minerals in the system Mg2SiO4 - Fe2SiO4 - SiO2
 * Contributions to Mineralogy and Petrology, 102: 41-68
 */

#define WHANPH (4.184*1000.0*3.21)  /* joules */
 ...
 */
#define R          8.3143
#define S          -R*(xan*log(xan)+xph*log(xph))
#define H          WHANPH*xan*xph
#define V          0.0
#define G          H - t*(S) + (p-1.0)*(V)

#define DGDR0      R*t*(log(xan)-log(xph)) + WHANPH*(xph-xan)
#define DGDT       -(S)
#define DGDP       (V)

#define D2GDR0R0   R*t*(1.0/xan+1.0/xph) - 2.0*WHANPH
#define D2GDR0DT   R*(log(xan)-log(xph))
#define D2GDR0DP   0.0
#define D2GDT2     0.0
#define D2GDTDP    0.0
#define D2GDP2     0.0

#define D3GDR0R0R0 R*t*(1.0/SQUARE(xph) - 1.0/SQUARE(xan));
#define D3GDR0R0DT R*(1.0/xan+1.0/xph)
						  ...
						  r:(double [NA])r
{
	...
	return [NSString stringWithFormat:@"K(Fe''%4.2fMg%4.2f)3AlSi3O10(OH)2", totFe2, totMg];
}
...
{
	double xan = (r[0]     > DBL_EPSILON) ? r[0]     : DBL_EPSILON;
	double xph = (1.0-r[0] > DBL_EPSILON) ? 1.0-r[0] : DBL_EPSILON;
