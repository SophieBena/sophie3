...
@implementation OrthoamphiboleBerman
...
+(void)initialize {
	if (self == [OrthoamphiboleBerman class]) {
		BOOL debug = false; // [[NSUserDefaults standardUserDefaults] boolForKey:@"DEBUG.SATLOOP.VERBOSE"];
		if (debug) NSLog(@"Initialize(OrthoamphiboleBerman) - entry ...");
...
-(id)init {
	if ((self = [super init])) {
		BOOL debug = false; // [[NSUserDefaults standardUserDefaults] boolForKey:@"DEBUG.SATLOOP.VERBOSE"];
		if (debug) NSLog(@"init(OrthoamphiboleBerman) ... entry ...");
